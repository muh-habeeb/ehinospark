...
export default function AnnouncementMarquee({ announcements }: AnnouncementProps) {
  if (!announcements.length) return null;

  // Create a continuous string of all announcements with gaps
  const marqueeContent = announcements.map(announcement => announcement.text).join("  |   ");
  ...
  return (
    <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white py-3 overflow-hidden relative mt-16 hover-pause-marquee">
      <div className="flex items-center">
        <span className="bg-white text-blue-600 px-3 py-1 rounded-full text-sm font-bold mr-4 flex-shrink-0 z-10 relative animate-pulse">
          📢 ANNOUNCEMENTS
        </span>
